...
#-*- coding: utf-8 -*-
"""
Scan for serial ports. Windows specific variant that also includes USB/Serial
adapters.
...
def basiccomlist(bool=True):
    lst = []
    for p in comports(bool):
        if p[1][0:3] == 'COM':
            lst.append(p[1])
    return lst
...
if __name__ == '__main__':
    ...
    for order, port, desc, hwid in comports():
        print port
    print 'REGRISTRY PORTS:'
    for order, port, desc, hwid in sorted(comports(False)):
        print port
    print 'AVAILABLE BASIC COM LIST'
    print basiccomlist()
    print 'AVAILABLE BASIC REGISTRY COM LIST'
    print basiccomlist(False)
